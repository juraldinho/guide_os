...
_MSG_NOT_FOUND = "Продажа не найдена."
...
def _sale_to_api(sale) -> dict:
    status = getattr(sale.status, "value", sale.status)
    payment = getattr(sale.payment_method, "value", sale.payment_method)
    return {
        "id": sale.sale_id,
        ...
        "createdAt": _utc_iso(sale.created_at),
        "updatedAt": _utc_iso(sale.updated_at),
    }
...
def register_guideshop_sales_routes(app: web.Application) -> None:
    async def list_sales_handler(request: web.Request) -> web.Response:
        ...
        try:
            async with provider.service_for(user_id) as service:
                response = await service.list_official_sales(cursor)
        except _GUIDESHOP_ROUTE_ERRORS as exc:
            return _guideshop_error_response(exc, rid)
        return success_response(
            {
                "sales": [_sale_to_api(item) for item in response.data],
                "page": _page_to_api(response),
            },
            rid,
        )

    async def get_sale_handler(request: web.Request) -> web.Response:
        rid, user_id, failure = _auth_or_error(request)
        if failure is not None:
            return failure
        sale_id = _safe_opaque_id(request.match_info["saleId"])
        if sale_id is None:
        ...
        try:
            async with provider.service_for(user_id) as service:
                sale = await service.get_official_sale(sale_id)
        except _GUIDESHOP_ROUTE_ERRORS as exc:
            return _guideshop_error_response(exc, rid)
        if sale is None:
            return _not_found_response(rid)
        return success_response(_sale_to_api(sale), rid)

    app.router.add_get("/app/v1/guideshop/sales", list_sales_handler)
    app.router.add_get(
        "/app/v1/guideshop/sales/{saleId}",
        get_sale_handler,
    )
