...
from database.queries import claim_guide_shop_lifecycle_jti
from services.guide_shop_settings import GuideShopStagingLifecycleSettings
from utils.guide_os_identity import is_canonical_guide_os_id


ALGORITHM = "EdDSA"
TOKEN_TYPE = "guide-os-staging-lifecycle+jwt"
ISSUER = "guide-os-staging-lifecycle"
AUDIENCE = "guide-os-staging-lifecycle"
SCOPE_ISSUE = "guide:link:issue"
SCOPE_CONFIRM = "guide:link:confirm"
SCOPE_REVOKE = "guide:link:revoke"
ALLOWED_SCOPES = {SCOPE_ISSUE, SCOPE_CONFIRM, SCOPE_REVOKE}
MAX_TTL_SECONDS = 60
...
class GuideShopStagingLifecycleAuthenticationError(Exception):
    pass


@dataclass(frozen=True)
class AuthenticatedStagingGuidePrincipal:
    guide_os_id: str
    scope: str
    kid: str
    issued_at: int
...
class GuideShopStagingLifecycleJWTVerifier:
    def __init__(
        self, settings: GuideShopStagingLifecycleSettings, *, clock=None
    ) -> None:
        if (
            not isinstance(settings, GuideShopStagingLifecycleSettings)
            or not settings.enabled
        ):
            raise GuideShopStagingLifecycleAuthenticationError(
                "Staging lifecycle authentication failed"
            )
        self._keys = {}
        try:
    ...
    ) -> AuthenticatedStagingGuidePrincipal:
        try:
            if (
            ...
            if (
                ...
                or not is_canonical_guide_os_id(claims["sub"])
            ...
            not_before = (
                self._numeric_date(claims["nbf"]) if "nbf" in claims else None
            )
            now = self._now()
            ...
            claimed = claim_guide_shop_lifecycle_jti(
            ...
            return AuthenticatedStagingGuidePrincipal(
                guide_os_id=claims["sub"],
                scope=claims["scope"],
                kid=header["kid"],
                issued_at=issued_at,
